#!/usr/bin/env python
"""
Quick tool to mass check port 80/tcp & 443/tcp HTTP connectivity. Requires: tqdm, futures, requests & requests[security]
"""
import requests
import argparse
import logging
...
def http_check(url, t, index, output):
    logging.debug("Attempting HTTP connection to %s" % url)

    try:
        r = requests.get("http://" + url, timeout=t, allow_redirects=False, verify=False)
        ...
        output[index] = "? - Unknown Error"
        logging.debug("%s: Error - %r" % (url, e))
        pass


def https_check(url, t, index, output):
    logging.debug("Attempting HTTPS connection to %s" % url)
...
def site_scan(url, t, pbar, index, output):
    logging.debug("Checking live status of %s" % url)

    http_check(url, t, index, output)
    https_check(url, t, index, output)
